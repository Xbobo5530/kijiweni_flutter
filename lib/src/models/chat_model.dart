...
abstract class ChatModel extends Model {
  ...
  Future<StatusCode> reportChat(Chat chat, User user)async{
    ...
    _database.runTransaction((transaction)async{
      ...
      await transaction.update(
           ...
           {REPORTS_FIELD: freshSnapshot[REPORTS_FIELD] + 1}).catchError(
             (error){
  ...
  /// updates the time of the last message sent to a community
  /// this can be used to sort communities based on the lase message sent
  Future<void> _updateCommunityLastMessageAt(Chat chat)async{
    Map<String, int> lastMessageMap = {
      LAST_MESSAGE_AT_FIELD : chat.createdAt
    };
    _database.collection(COMMUNITIES_COLLECTION)
    .document(chat.communityId)
    .updateData(lastMessageMap)
    .catchError((error)=>
    print('error on updating community last message at: $error')
    );
  }
}
